...
export default function ZepAuditModal({
  ...
  const modalMarkup = (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4">
      <div className="bg-black border border-white/10 rounded-xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden">
        
        {/* HEADER */}
        <header className="px-6 py-4 border-b border-white/8 flex justify-between items-center bg-black">
          <div>
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white p-1 rounded-md hover:bg-white/6 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </header>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0 bg-black">
              ...
              {aiIncidentList.length === 0 ? (
                ...
                aiIncidentList.map((diag, index) => {
                  ...
                  return (
                      <div
                        key={diag.id || index}
                        className="bg-[#050505] border border-white/8 rounded-lg p-4 flex flex-col gap-3"
                      >
                      <div className="flex justify-between items-center text-[10px] font-mono border-b border-white/8 pb-2">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-start">
                        <div className="text-[10px] font-mono bg-black border border-white/8 p-3 rounded shrink-0 flex flex-col gap-1 text-slate-400">
                          <div>
                        ...
                        <div className="flex-1 bg-black border border-white/8 rounded p-3 text-xs text-slate-300 font-sans leading-relaxed">
          ...
          <section className="w-full md:w-[380px] h-[35vh] md:h-full p-5 md:p-6 flex flex-col bg-black shrink-0 min-h-0 overflow-hidden border-t md:border-t-0 md:border-l border-white/8">
            <div className="flex items-center gap-2 mb-4 shrink-0">
              <Network className="w-4 h-4 text-white/75" />
              <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
            ...
            <div className="flex-1 overflow-y-auto pr-1">
              {isLoadingZep ? (
                ...
                <div className="h-full flex items-center justify-center border border-dashed border-white/10 rounded-lg p-6 text-center">
                ...
                <div className="space-y-3">
                  {zepFacts.map((fact, index) => (
                    <div
                      key={index}
                      className="bg-[#050505] border border-white/8 rounded-lg p-3 text-[11px] text-slate-300 font-mono leading-relaxed"
                    >
        ...
        {/* FOOTER */}
        <footer className="px-6 py-3 border-t border-white/8 flex justify-end items-center bg-black">
          <button
            onClick={onClose}
            className="bg-black border border-white/10 hover:border-white/20 text-slate-300 px-4 py-1.5 rounded-md text-xs font-bold transition-all"
          >
            Cerrar Auditoría
          </button>
        </footer>
