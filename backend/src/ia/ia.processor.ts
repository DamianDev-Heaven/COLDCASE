...
export class IaProcessor extends WorkerHost {
  ...
  async process(
    job: Job<
      { viajeId: string; incidenteData: TelemetriaInput },
      unknown,
      string
    >,
  ): Promise<unknown> {
    ...
    if (job.name === 'analyze-incident') {
      ...
      try {
        this.logger.log(
          `[Job ${job.id}] Ejecutando análisis de IA en segundo plano para viaje ${viajeId}...`,
        );

        const iaResult =
          await this.iaAnalysisService.analizarEventoEnTiempoReal(
            viajeId,
            incidenteData,
          );

        this.logger.log(
          `[Job ${job.id}] Análisis completado y persistido. Diagnóstico: ${iaResult.diagnostico_tecnico}`,
        );
        return iaResult;
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        this.logger.error(
          `Error al procesar diagnóstico de incidente en segundo plano: ${message}`,
        );
        throw err;
      }
    }
